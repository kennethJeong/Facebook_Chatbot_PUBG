...
*/
function TypingOn($userkey=NULL)
{
...
function TypingOff($userkey=NULL)
{
	global $senderID, $accessToken;
...
function messageWebview($send, $userKey=NULL, $messaging_type=NULL) 
{
	global $senderID, $accessToken;
...
function messageImageList($send, $userKey=NULL, $messaging_type=NULL) 
{
	global $senderID, $accessToken;
	...
	if($send['title'] && $send['imgUrl'] && count($send['title']) == count($send['imgUrl'])) {
		$message = array
		(
			"recipient" => array
			(
		...
		$countTitle = count($send['title']);
		$countImg = count($send['imgUrl']);
		for($i=0; $i<$countTitle; $i++) {
			$elementsArrayPush[$i] = array
			(
				"title" => $send['title'][$i],
			...
			array_push($message["message"]["attachment"]["payload"]["elements"], $elementsArrayPush[$i]);
		}		
